...
class WantedService:
    ...
    def bind_sync_ledger(self, sync_ledger) -> None:
        self._sync_ledger = sync_ledger

    def bind_external(self, external) -> None:
        self._external = external

    def bind_job_executor(self, job_executor) -> None:
        self._job_executor = job_executor
    ...
    def _last_sync_path(self) -> Path:
        ensure_wanted_layout()
        return WANTED_ROOT / _LAST_SYNC_NAME
    ...
    def list_tracks(self) -> list[WantedTrackResponse]:
        rows = self._repo.list_all()
        matched_rows = [r for r in rows if r.relative_path]
        unmatched_rows = [r for r in rows if not r.relative_path]
        out: list[WantedTrackResponse] = []
        for i, row in enumerate(matched_rows, start=1):
            out.append(self._to_response(row, display_index=f"W{i}"))
        for row in unmatched_rows:
            out.append(self._to_response(row, display_index=None))
        return out
    ...
    def delete_track(self, track_id: UUID, *, mode: str) -> None:
        ...
        with self._lock:
            ...
            path = WANTED_ROOT / row.relative_path
            if mode == "wipe_list":
                self._unlink_wanted_file(path)
                self._repo.delete(track_id)
                return
            if mode == "to_raw_delete":
                self._move_to_raw_delete(path)
                self._repo.delete(track_id)
                return
    ...
    def _unlink_wanted_file(self, path: Path) -> None:
    ...
    def try_link_local(self, track_id: UUID) -> WantedTrackResponse:
        row = self._repo.get(track_id)
        if row is None:
            raise FileNotFoundError("wanted track not found")
    ...
    def _fulfill_to_direct(self, row: WantedTrack, video_id: str) -> dict:
        """Move fulfilled wish into Direct then drop wishlist row."""
    ...
    def _find_strict_local(
        ...
        *,
        ...
        """Scan download + external for exact normalized triple; prefer existing files."""
        if not title_norm or not artist_norm:
            return None
        roots = [DOWNLOAD_ROOT]
        if EXTERNAL_RAW_ROOT.is_dir():
            roots.append(EXTERNAL_RAW_ROOT)
        from yubal.utils.library import EXTERNAL_ORGANIZED_ROOT

        if EXTERNAL_ORGANIZED_ROOT.is_dir():
            roots.append(EXTERNAL_ORGANIZED_ROOT)
        for root in roots:
            if not root.is_dir():
                continue
            for path in root.rglob("*"):
                if not path.is_file() or path.suffix.lower() not in AUDIO_SUFFIXES:
                    continue
                # Prefer embedded tags via mutagen if available; else filename heuristics skip
                tags = self._read_tags(path)
                if tags is None:
                    continue
                t_n, a_n, al_n = tags
                if t_n == title_norm and a_n == artist_norm:
                    if album_norm and al_n != album_norm:
                        continue
                    if not album_norm and al_n:
                        # Wishlist without album only matches files without album
                        continue
                    return path
        return None
    ...
    def _move_to_raw_delete(self, path: Path) -> None:
        if not path.is_file():
            return
        ...
        lrc = path.with_suffix(".lrc")
        # path already moved — sidecar beside original name under wanted
        wanted_lrc = Path(str(path) + "").with_suffix(".lrc") if False else None
        _ = wanted_lrc
        sidecar = Path(str(path.parent / (path.stem + ".lrc")))
        # After rename, original path is gone; look next to dest
        src_lrc = dest.with_suffix(".lrc")
        # Try original sibling before move — already gone. Best-effort: ignore.
        _ = (lrc, sidecar, src_lrc)
